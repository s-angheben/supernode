...
from models.procedure import train, test, printDataInfo
...
dataset = TUDataset(root=path, name=dataset_name, pre_transform=T.OneHotDegree(135))
...
concepts_list_ex = [
       {"name": "GCB", "fun": cycle_basis, "args": [20]},
       {"name": "GMC", "fun": max_cliques, "args": []},
       {"name": "GLP2", "fun": line_paths, "args": [4]}
    ]
print(concepts_list_ex)

#dataset.transform = AddSupernodes(concepts_list_ex)
transf = T.Compose([T.OneHotDegree(135), AddSupernodes(concepts_list_ex)])
dataset = TUDataset(root=pathT, name=dataset_name,
                    pre_transform=transf
                    )
