...
from data.dataset import TUDIMDBbDataModule, makefeatures
...
dataset_name = f"IMDBb_supernode_homogenous_{hash_name}"

transformation = T.Compose([makefeatures, AddSupernodes(concept_list)])

dm = TUDIMDBbDataModule(f'./dataset/{dataset_name}', batch_size=BATCH_SIZE,
...
logger = TensorBoardLogger("tb_logs/" + "IMDBb_hom", model_log["model"], concept_list_name)
logger.log_hyperparams(dict({"device": device, "concepts": concept_list_name,
                        "dataset": "IMDBb", "epochs": MAX_EPOCH}, **model_log))
